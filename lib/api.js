...
//    return all the availble contract details at last 2pm time(timestamp) slots
  ...
  api.get('/premium/diff/:sym', function (req, res) {
    if (req.params.sym && (STARTED == 0)) {
      STARTED++;
      var SQL1 = format('SELECT * from s__%I where tstamp in (SELECT min(tstamp) from s__%I where date_trunc(\'hour\', tstamp) in (select max( date_trunc(\'hour\', tstamp) ) from s__%I group by date_trunc(\'day\', tstamp)) group by date_trunc(\'hour\', tstamp)) order by tstamp', req.params.sym, req.params.sym, req.params.sym);

      console.log('~', STARTED, SQL1);
      _.once(client.query(SQL1,  function (err, result) {
        if (err) {
          console.log('error running query', err);
          res.send('error running query', err);
        }
          ...
          if (lastval == null) {
            ...
            client.query(SQL,  function (err, rtn) {
              if (err) {
                ...
                item.diff = item.last - lastval.last;
                item.sum = rtn.rows[0].sum;
                console.log(item.id);
...
console.log('done!');
          fs.writeFileSync('data'+req.params.sym+'.xlsx', json2xls(finalResult), 'binary');
          res.xls('diff_'+req.params.sym+'.xlsx', finalResult);
        });
      }));
    } else {
      STARTED = 0;
