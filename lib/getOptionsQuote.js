  ...
  requestWithEncoding(options, function(err, data) {
    if (err) callback(err);
      ...
      if (config.get('saveHTML') == 1) {
        fs.writeFile('./html/'+options.symbol+'~'+options.action+'^'+dt.getTime(), data, function (err) {
          if (err) throw err;
          console.log('Saved original HTML file: ' + options.symbol+'~'+options.action+'^'+dt.getTime());
        });
      }
...
var optionExtraction = function(options, callback) {
  requestWithEncoding(options, function(err, data) {
    ...
    if (err) console.log(err);
      ...
      var dt = new Date(), header = [], rtnData = [];
      var $ = cheerio.load(data);
      if (config.get('saveHTML') == 1) {
        fs.writeFile('./html/'+options.symbol+'^'+dt.getTime(), data, function (err) {
          if (err) throw err;
...
exports.getOptionsQuote = function(newSymbol, callback) {
  ...
  async.parallel({
      CallGreeks : function(cb) {
        ...
        greeksExtraction(greeksAnalytical('C', newSymbol), function(err, data, tick) {
          if (err) cb(err);
          else {
            /*
...
// get the Put data second
        greeksExtraction(greeksAnalytical('P', newSymbol), function(err, data, tick) {
          if (err) cb(err);
          else {
            /*
            _.map(data, function(item) {
             ...
             cb(null, data, tick);
            */
            console.log('~2~', data, tick);
            cb(null, [data, tick]);
          };
        });
      },
      CallPut : function(cb) {
        optionExtraction(optionsPrice(newSymbol), function(err, data, tick) {
          if (err) cb(err);
          else {
            console.log('~3~', data, tick);
            cb(null, [data, tick]);
...
/*
    ...
    function(err, results) {
      console.log('Parsing completed ', newSymbol);
//      console.log(results);
