  ...
  requestCall((typeof fullpath == 'undefined') ? options : fullpath, function(err, data) {
    if (err) callback(err);
      ...
      if (config.get('saveHTML') == 1) {
        fs.writeFile(GLOBAL.rootpath + '/html/'+options.symbol+'~'+options.action+'^'+dt.getTime(), data, function (err) {
          if (err) throw err;
...
var optionExtraction = function(options, fullpath, callback) {
  ...
  requestCall((typeof fullpath == 'undefined') ? options : fullpath, function(err, data) {
    if (err) console.log(err);
      ...
      var $ = cheerio.load(data);
      if (config.get('saveHTML') == 1) {
        fs.writeFile(GLOBAL.rootpath + '/html/'+options.symbol+'^'+dt.getTime(), data, function (err) {
          if (err) throw err;
