    ...
    if (err) callback(err);
      ...
      if (config.get('saveHTML') == 1) {
        fs.writeFile('./html/'+options.symbol+'~'+options.action+'^'+dt.getTime()+'.html', data, function (err) {
          if (err) throw err;
            if (config.get('debug') == 1) console.log('Saved original HTML file: ' + options.symbol+'~'+options.action+'^'+dt.getTime()+'.html');
        });
      }
    ...
    if (err) console.log(err);
      ...
      if (config.get('saveHTML') == 1) {
        fs.writeFile('./html/'+options.symbol+'^'+dt.getTime()+'.html', data, function (err) {
          if (err) throw err;
            if (config.get('debug') == 1) console.log('Saved original HTML CallPut file: ' + options.symbol+'^'+dt.getTime());
        });
      }
