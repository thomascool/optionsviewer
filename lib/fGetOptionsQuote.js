...
var greeksExtraction = function(options, fullpath, callback) {
  ...
  requestFile(fullpath, function(err, data) {
    if (err) callback(err);
    else {
...
exports.getOptionsQuote = function(newSymbol, filenames, callback) {
  async.parallel({
      CallGreeks : function(cb) {
        ...
        greeksExtraction(greeksAnalytical('C', newSymbol), pathName[0].fullpath, function(err, data, tick) {
          if (err) cb(err);
          else {
...
// get the Put data second
        ...
        greeksExtraction(greeksAnalytical('P', newSymbol), pathName[0].fullpath, function(err, data, tick) {
          if (err) cb(err);
          else {
            cb(null, {data:data, tick:tick});
          };
      ...
      CallPut : function(cb) {
        ...
        optionExtraction(optionsPrice(newSymbol), pathName[0].fullpath, function(err, data, tick) {
          if (err) cb(err);
          else {
            cb(null, {data:data, tick:tick});
          };
